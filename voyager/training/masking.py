...
def action_mask_from_info(info: Mapping[str, object]) -> np.ndarray:
...
def stack_action_masks(
    ...
    return np.stack(
        [action_mask_from_info(infos[agent_id]) for agent_id in agent_ids],
        axis=0,
    )
...
def mask_numpy_logits(logits: np.ndarray, action_mask: np.ndarray) -> np.ndarray:
    ...
    if logits_array.shape != (ACTION_COUNT,):
        raise ValueError(f"logits must have shape ({ACTION_COUNT},), got {logits_array.shape}.")
    if mask.shape != logits_array.shape:
        raise ValueError(f"action_mask shape {mask.shape} does not match logits {logits_array.shape}.")
    if not np.any(mask):
        raise ValueError("action_mask must allow at least one action.")
